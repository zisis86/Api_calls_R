...
##################################### try alternatives to run the method without error ########################################################
run_bim <- function(headers, execution_info) {    
  cat('-- BioInfoMiner execution (please wait).. --\n')
  
  response <- GET(
    paste0(base_url, 'runbim'), 
  ...
  print(headers(response))  # Use print() for lists instead of cat()
  cat("Response Content:\n", content(response, "text"), "\n")
  
  if (status_code(response) == 200) {
    cat("Successful execution!\n")
  } else {
    cat("Error during execution.\n")
  }
}
